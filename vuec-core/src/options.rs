...
pub struct ParserOptions {
    ...
    pub is_native_tag: Option<fn(tag: &String) -> bool>,
    /// e.g. native elements that can self-close, e.g. `<img>`, `<br>`, `<hr>`
    pub is_void_tag: fn(tag: &String) -> bool,
    /// e.g. elements that should preserve whitespace inside, e.g. `<pre>`
    pub is_pre_tag: fn(tag: &String) -> bool,
    /// Platform-specific built-in components e.g. `<Transition>`
    pub is_builtin_component: Option<fn(tag: &String) -> Option<RuntimeHelper>>,
    /// Separate option for end users to extend the native elements list
    pub is_custom_element: fn(tag: &String) -> bool,
    /// Get namespace of an element
    pub get_namespace: fn(tag: &String, parent: Option<ElementNode>) -> NameSpace,
    /// Get text parsing mode for this element
    pub get_text_mode: fn(tag: &ElementNode, parent: Option<ElementNode>) -> TextMode,
    /// Only needed for DOM compilers
    pub decode_entities: fn(raw_text: String, as_attr: bool) -> String,
...
pub struct SharedTransformCodegenOptions {
    ...
    /// and cannot use `with`
    /// @default mode === 'module'
    prefix_identifiers: Option<bool>,
...
pub struct TransformOptions {
    ...
    error_handling_options: Option<ErrorHandlingOptions>,

    /// An array of node transforms to be applied to every AST node.
    node_transforms: Option<Vec<NodeTransform>>,
    /// An object of { name: transform } to be applied to every directive attribute
    /// node found on element nodes.
    directive_transforms: Option<HashMap<String, DirectiveTransform>>,
    /// An optional hook to transform a node being hoisted.
    /// used by compiler-dom to turn hoisted nodes into stringified HTML vnodes.
    /// @default null
    transform_hoist: Option<HoistTransform>,
...
pub struct CodegenOptions {
    codegen_options: Option<SharedTransformCodegenOptions>,
    /// - `module` mode will generate ES module import statements for helpers
    /// and export the render function as the default export.
    /// - `function` mode will generate a single `const { helpers... } = Vue`
    /// statement and return the render function. It expects `Vue` to be globally
